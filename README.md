...
- 支持多种消息类型的数据提取：
  ...
  - `sensor_msgs/PointCloud2` - 点云数据（支持八叉树建图）
- 不支持的话题类型点击时弹窗提示
- 数据导出为CSV格式
...
- 基于点云数据构建八叉树空间索引
- 自动检测sensor_msgs/PointCloud2话题，优先使用点云数据建图
- 支持从位姿话题（Odometry/PoseStamped等）构建点云（备选方案）
- 可配置的体素大小、递归深度、密度阈值等参数
- 高度阈值分层显示（支持剥离顶部冗余结构）
- 多种颜色映射方案：按高度/密度/深度/自定义
- 透明度调节、边界线显示控制、渲染质量配置
- 3D体素可视化（交互式旋转/缩放/平移）
- 2D俯视投影图
- 统计分析图表（深度分布、点数分布、高度分布）
- 体素数据导出（NPZ格式）
- 点查询和射线投射功能
- 综合仪表板视图
- 图表内嵌预览与导航工具栏
...
- `geometry_msgs/PoseWithCovarianceStamped` - 带协方差的位姿
- `nav_msgs/Path` - 路径消息（提取最新位置点）
- `geometry_msgs/Twist` - 速度数据（线速度+角速度）
- `geometry_msgs/TwistStamped` - 带时间戳的速度
- `geometry_msgs/Vector3Stamped` - 三维向量
- `sensor_msgs/Imu` - IMU数据
...
#### 2. 姿态-时间曲线图 (orientation\_time.png)
...
### v2.2.0 (2026-04-27)
- ✅ 新增八叉树地图生成模块（octree_map.py）
- ✅ 新增八叉树可视化模块（octree_visualizer.py）
- ✅ 新增sensor_msgs/PointCloud2话题支持（点云数据提取）
- ✅ 八叉树建图自动检测PointCloud2话题，优先使用点云数据
- ✅ 支持体素大小、深度、密度阈值等参数配置
- ✅ 支持高度阈值分层显示（剥离顶部冗余结构）
- ✅ 支持4种颜色映射方案（高度/密度/深度/自定义）
- ✅ 支持透明度、边界线、渲染质量等可视化配置
- ✅ 3D体素渲染、2D俯视投影、统计分析图表
- ✅ 体素数据导出（NPZ格式）
- ✅ 点查询和射线投射功能
- ✅ 35个单元测试全部通过
- ✅ UI集成：新增八叉树选项卡和参数配置面板

### v2.1.0 (2026-04-27)

- ✅ 修复nav\_msgs/Path类型2D/3D图连线到原点的问题
- ✅ 新增geometry\_msgs/Twist类型数据提取支持（线速度+角速度）
- ✅ 常用话题改为动态话题列表（根据bag文件自动生成）
- ✅ 不支持的话题类型灰色显示并弹窗提示
- ✅ 快速操作模块改为"绘图功能"模块
- ✅ 新增"数据导出"独立模块
- ✅ 新增is\_topic\_supported()方法检查话题支持状态

### v2.0.0 (2026-04-27)

- ✅ 3D交互式轨迹可视化（独立选项卡）
- ✅ 界面重组为左侧纵向选项卡（加载数据+数据处理）
- ✅ 新增INFO日志窗口（捕获控制台输出）
- ✅ 修复QTextCursor跨线程错误
- ✅ 修复QListWidget的setItemSelected属性错误
- ✅ 修复FilterConfig不可哈希类型错误
- ✅ 图表内嵌预览功能（matplotlib canvas嵌入Qt）
- ✅ 中文字体自动适配
- ✅ 重新加载bag文件时完全数据清除
