...
class BagParser:
    ...
    def extract_pointcloud2_data(self, topic_name: str) -> Optional[np.ndarray]:
        """从sensor_msgs/PointCloud2话题提取点云数据
        
        解析PointCloud2二进制数据，提取所有点的xyz坐标。
        支持rosbag后端（直接访问msg对象）和rosbags后端。
        
        Args:
            topic_name (str): PointCloud2话题名称
            
        Returns:
            Optional[np.ndarray]: 形状为(N, 3)的点云数组，失败返回None
            
        PointCloud2消息结构：
            - header: 时间戳和坐标系
            - height/width: 点云高度和宽度
            - fields: 字段描述列表（x, y, z, rgb等）
            - point_step: 单个点的字节步长
            - data: 原始二进制数据
        """
        if topic_name not in self.raw_data or len(self.raw_data[topic_name]) == 0:
            print(f"[WARN] 话题 '{topic_name}' 未找到或无数据")
            return None
        
        msg_type = self.topics_info.get(topic_name)
        if msg_type is not None and 'PointCloud2' not in msg_type.msg_type:
            print(f"[WARN] 话题 '{topic_name}' 不是PointCloud2类型")
            return None
        ...
        print(f"\n正在从话题 '{topic_name}' 提取点云数据...")
        print(f"  消息总数: {len(self.raw_data[topic_name])}")
        
        for idx, (timestamp, msg) in enumerate(self.raw_data[topic_name]):
            try:
                if self.backend == 'rosbags':
                    msg = self._deserialize_rosbags_message(msg, topic_name)
                    if msg is None:
                ...
                points = self._extract_points_from_pc2(msg)
                ...
                error_count += 1
                if error_count <= 3:
                    print(f"  [WARN] 处理消息 #{idx+1} 时出错: {str(e)[:80]}")
                continue
        
        total = success_count + error_count
        print(f"  点云提取统计: 成功 {success_count}/{total}")
        
        if not all_points:
            print("  [ERROR] 未能从任何消息中提取点云数据")
            return None
        
        result = np.vstack(all_points)
        print(f"  [OK] 提取点云总数: {len(result)} 个点")
        return result
    
    def _extract_points_from_pc2(self, msg: Any) -> Optional[np.ndarray]:
        """从单个PointCloud2消息对象中提取xyz点云
        
        解析PointCloud2的二进制data字段，根据fields描述
        定位x/y/z字段的偏移量，逐点提取坐标。
        
        Args:
            msg: ROS PointCloud2消息对象
            
        Returns:
            Optional[np.ndarray]: 形状为(N, 3)的点云数组，失败返回None
        """
        try:
            if not hasattr(msg, 'data') or not hasattr(msg, 'fields'):
                return None
            ...
            expected_len = n_points * point_step
            ...
            for i in range(n_points):
                base = i * point_step
                try:
                    x = struct.unpack_from('f', raw_bytes, base + x_offset)[0]
                    y = struct.unpack_from('f', raw_bytes, base + y_offset)[0]
                    z = struct.unpack_from('f', raw_bytes, base + z_offset)[0]
