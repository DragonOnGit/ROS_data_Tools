...
class MainWindow(QMainWindow):
    ...
    def _create_left_panel(self) -> QWidget:
        ...
        load_layout.addWidget(topic_group)
        
        info_group = QGroupBox("ℹ️ INFO日志")
        ...
        info_layout.addWidget(self.text_console)
        load_layout.addWidget(info_group)
        
        self.left_tab_widget.addTab(load_tab, "加载数据")
        ...
        btn_plot_3d = QPushButton("🌐 绘制3D轨迹")
        btn_plot_3d.clicked.connect(self.plot_3d_trajectory)
        ...
        self.left_tab_widget.addTab(process_scroll, "数据处理")
    ...
    def _clear_3d_canvas(self):
        if self.canvas_3d is not None:
            self.tab_3d_layout.removeWidget(self.canvas_3d)
            self.canvas_3d.deleteLater()
            self.canvas_3d = None
        if self._toolbar_3d is not None:
            self.tab_3d_layout.removeWidget(self._toolbar_3d)
            self._toolbar_3d.deleteLater()
            self._toolbar_3d = None
        if self.lbl_3d_placeholder.parent() is None:
            self.tab_3d_layout.addWidget(self.lbl_3d_placeholder)
        self.lbl_3d_placeholder.show()
    ...
    def open_bag_file(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择ROS Bag文件", "", "Bag文件 (*.bag);;所有文件 (*)"
        )
        if file_path:
    ...
    def select_target_topic(self, topic_name: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"提取数据时发生错误:\n{str(e)}", QMessageBox.Ok)
    
    # ==================== Filter Operations ====================
    
    def get_current_filter_config(self) -> FilterConfig:
        type_map = {
            0: 'moving_average', 1: 'weighted_moving_average',
            2: 'exponential_moving_average', 3: 'median',
            4: 'kalman', 5: 'lowpass_butterworth', 6: 'savitzky_golay'
        }
        config = FilterConfig(
    ...
    def apply_filter_to_selected(self):
        if not self.current_pose_data:
            QMessageBox.warning(self, "提示", "请先选择并提取至少一个话题的位姿数据！", QMessageBox.Ok)
            return
        ...
        try:
            for topic_name, pose_data in self.current_pose_data.items():
                ...
                filtered_pose = self.filter_processor.apply_filter_to_pose_data(pose_data, config)
                ...
                for field in fields:
                    ...
                    if len(original) > 0 and len(filtered) > 0:
                        stats = self.filter_processor.get_filter_comparison_stats(original, filtered)
                        report_lines.append(
                            f"\n  字段: {field.upper()}"
            ...
            QMessageBox.information(
                self, "滤波完成",
                f"已对所有选中的话题应用 {filter_name} 滤波器。\n\n请在'滤波对比'标签页查看详细效果和对比图表。",
                QMessageBox.Ok
            )
        except Exception as e:
            import traceback
            traceback.print_exc()
            QMessageBox.critical(self, "滤波错误", f"应用滤波器时发生错误:\n{str(e)}", QMessageBox.Ok)
        ...
        for t_idx, topic_name in enumerate(topics):
            ...
            for f_idx, field in enumerate(fields):
                ...
                if len(raw_values) > 0 and len(filt_values) > 0:
                    ...
                    ax.plot(time_raw[:min_len], raw_values[:min_len], 'b-', label='原始', linewidth=1.0, alpha=0.7)
                    ax.plot(time_raw[:min_len], filt_values[:min_len], 'r--', label='滤波', linewidth=1.0, alpha=0.7)
    ...
    def _display_3d_figure(self):
        ...
        for idx, (topic_name, pose_data) in enumerate(self.current_pose_data.items()):
            color = colors[idx % len(colors)]
            
            if len(pose_data.x) > 0:
                ax.plot(pose_data.x, pose_data.y, pose_data.z,
                       color=color, linestyle='-', label=f'{topic_name} (原始)',
                       linewidth=1.5, alpha=0.8)
                ax.scatter([pose_data.x[0]], [pose_data.y[0]], [pose_data.z[0]],
                          color=color, marker='o', s=100, zorder=5, edgecolors='black')
                ax.scatter([pose_data.x[-1]], [pose_data.y[-1]], [pose_data.z[-1]],
                          color=color, marker='s', s=100, zorder=5, edgecolors='black')
                
                if topic_name in self.filtered_pose_data:
                    filt = self.filtered_pose_data[topic_name]
                    if len(filt.x) > 0:
                        ax.plot(filt.x, filt.y, filt.z,
                               color=color, linestyle='--', label=f'{topic_name} (滤波)',
        ...
        self.tab_3d_layout.insertWidget(0, toolbar)
        self.tab_3d_layout.insertWidget(1, canvas)
        self.canvas_3d = canvas
        self._toolbar_3d = toolbar
        
        self.tab_widget.setCurrentIndex(2)
        self.statusBar().showMessage("3D轨迹图已生成 - 支持鼠标拖拽旋转、滚轮缩放、右键平移")
    ...
    def export_data(self):
        if not self.parser:
            QMessageBox.warning(self, "提示", "请先加载bag文件！", QMessageBox.Ok)
            return
        output_dir = QFileDialog.getExistingDirectory(self, "选择导出目录", "./exported_data")
        if output_dir:
            success = self.parser.export_topics_to_csv(output_dir)
            if success:
                QMessageBox.information(self, "导出成功", f"数据已成功导出至:\n{output_dir}", QMessageBox.Ok)
            else:
                QMessageBox.critical(self, "导出失败", "导出过程中出现错误，请检查日志。", QMessageBox.Ok)
    
    def save_all_plots(self):
        if not self.visualizer.figures:
            QMessageBox.information(self, "提示", "暂无图表可保存。请先生成一些图表。", QMessageBox.Ok)
            return
        output_dir = QFileDialog.getExistingDirectory(self, "选择保存目录", "./plots")
        if output_dir:
            saved_files = self.visualizer.save_all_plots(output_dir)
            QMessageBox.information(self, "保存成功", f"已保存 {len(saved_files)} 个图表至:\n{output_dir}", QMessageBox.Ok)
    
    # ==================== Utility ====================
    
    def _show_no_data_warning(self):
        QMessageBox.warning(self, "无数据", "请先加载bag文件并选择要分析的话题！", QMessageBox.Ok)
